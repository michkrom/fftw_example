...
int main() {
    ...
    FFTW1D<> fft(1<<5, FFTW_FORWARD, FFTW_MEASURE);

    // Create some example data
    double ph = 0;
    const double phInc = 2 * M_PI / fft.N;
    for_each(fft.in(), [&ph, phInc](auto& sample) {
    ...
    auto out = gsl::reinterpret_span<std::complex<double>>(fft.out());
    normalize(out);
